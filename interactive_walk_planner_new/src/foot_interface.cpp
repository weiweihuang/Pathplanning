...
#include <interactive_walk_planner/orientation.h>
  ...
  switch ( feedback->event_type )
  {
      ...
      case InteractiveMarkerFeedback::MENU_SELECT:
		...
		if(feedback->menu_entry_id ==1){
			...
			interactive_walk_planner::replan msg;
			msg.x = 1;
			...
			interactive_walk_planner::goal_pose goal_location;
			...
			for (int i=0; i<MAX_FOOT_NO; i++){
				...
				foot_sequence[2][7] = 1.0; //normal_z
		    		...
		    		foot_sequence[3][0] = 0.35;
		    		foot_sequence[3][1] = 0.0;
		    		foot_sequence[3][2] = 0;
		    		foot_sequence[3][3] = 0;
		    		foot_sequence[3][4] = 0.16;
				foot_sequence[3][5] = 0.0; //normal_x
			        foot_sequence[3][6] = 0.0; // normal_y
				foot_sequence[3][7] = 1.0; //normal_z
		    		printf( "The step 3 position is %g %g %g \n", foot_sequence[3][0], foot_sequence[3][1], foot_sequence[3][4]);
			    }
			    else if (i==4){
		    		foot_sequence[4][0] = 0.35;
		    		foot_sequence[4][1] = -0.20;
		    		foot_sequence[4][2] = 0;
		    		foot_sequence[4][3] = 1;
		    		foot_sequence[4][4] = 0.16;
				foot_sequence[4][5] = 0.0; //normal_x
			        foot_sequence[4][6] = 0.0; // normal_y
				foot_sequence[4][7] = 1.0; //normal_z
		    		printf( "The step 4 position is %g %g %g \n", foot_sequence[4][0], foot_sequence[4][1], foot_sequence[4][4]);
			    }
			...
			int count = 1;
			atlas_ros_msgs::AtlasWalkParams queue;
			while (foot_sequence[count][0]!=0 || foot_sequence[count][1]!=0 || foot_sequence[count][2]!=0){
				...
				step.position.x = foot_sequence[count][0]-0.045; // this is for foot sequence command given to ankle instead of center of the foot
				...
				queue.step_queue.push_back(step);
...
/*
			...
			for (int i=0; i<MAX_FOOT_NO; i++){
			    ...
			    foot_sequence[0][3] = 1;
			    if (i>0 && i<10){
			    	    foot_sequence[i][0] = 0.045;
			    	    foot_sequence[i][2] = 0;
				    if (foot_sequence[i-1][3] ==0){
					foot_sequence[i][3] = 1;
					foot_sequence[i][1] = -0.26;
				    }	
			    	    else{
					foot_sequence[i][3] = 0;
					foot_sequence[i][1] = 0.0;
				    }
			    	    foot_sequence[i][4] = 0.0;
				    foot_sequence[i][5] = 0.0; //normal_x
				    foot_sequence[i][6] = 0.0; // normal_y
				    foot_sequence[i][7] = 1.0; //normal_z
			    	    printf( "The step %d position is %g %g %g \n", i, foot_sequence[i][0], foot_sequence[i][1], foot_sequence[i][4]);
			    }
			}
...
// %EndTag(Menu)%
/*
void ori_estimationCallback(const interactive_walk_planner::orientation::ConstPtr& msg)
{
...
void foot_sequenceCallback(const interactive_walk_planner::foot_sequence::ConstPtr& msg)
{
  ...
  if (msg->header.stamp.toSec() < initTime.toSec()) {
    printf("\n\n\n\nbogus message, %g %g\n\n\n\n\n", msg->header.stamp.toSec(), initTime.toSec());
	  ...
	  for (int i=0; i<MAX_FOOT_NO; i++){
	    ...
	    foot_sequence[i][5] = msg->normal_x[i];
	    foot_sequence[i][6] = msg->normal_y[i];
	    foot_sequence[i][7] = msg->normal_z[i];	
	    ...
	    std::string s = ss.str();
	      ...
	      printf( "The step %d position is %g %g %g %g \n", i, foot_sequence[i][0], foot_sequence[i][1], foot_sequence[i][4], foot_sequence[i][2]);
              ...
              Eigen::Vector3d raw_normal(n_x, n_y, n_z);
	      Eigen::Vector3d normal = raw_normal.normalized();	
	      Eigen::Vector3d normal_cross = normal.cross(ori);	
	      Eigen::Quaterniond q(Eigen::AngleAxisd(-acosf(normal.dot(ori)), normal_cross));

              //printf( "The step %d quaternion are %g %g %g %g \n", i, q.w(), q.x(), q.y(), q.z());
	
	      position.orientation.w = cosf(foot_sequence[i][2]/2.);//q.w();	
	      position.orientation.x = 0;//q.x();
	      position.orientation.y = 0;//q.y();
	      position.orientation.z = sinf(foot_sequence[i][2]/2.);;//q.z();
	    if (msg->s[i] > 0)
	  	makeMovingMarker(position, s, false, true);
	    else
		makeMovingMarker(position, s, false, false);
...
//==================== make the last two step align with the cinder block =========================//
//===================== this will be a problem for z-zag ===================================//
/*
	    for(int i=0;i<2;i++){	     	
		    foot_sequence[max_no+i-1][2] = ori_estimation;
		    std::stringstream ss;
		    ss << max_no+i;
		    std::string s = ss.str();
		      geometry_msgs::Pose position;
		      position.position.x = foot_sequence[max_no+i-1][0];
		      position.position.y = foot_sequence[max_no+i-1][1];
		
		      position.position.z = foot_sequence[max_no+i-1][4];
		      float n_x = foot_sequence[max_no+i-1][5];
		      float n_y = foot_sequence[max_no+i-1][6];
		      float n_z = foot_sequence[max_no+i-1][7];	
		      printf( "The step %d position is %g %g %g %g\n", max_no+i-1, foot_sequence[max_no+i-1][0], foot_sequence[max_no+i-1][1], foot_sequence[max_no+i-1][4], foot_sequence[max_no+i-1][2]);
		      ...
		      position.orientation.w = cosf(foot_sequence[max_no+i-1][2]/2.);//q.w();	
		      position.orientation.x = 0;//q.x();
		      position.orientation.y = 0;//q.y();
		      position.orientation.z = sinf(foot_sequence[max_no+i-1][2]/2.);;//q.z();
		    if (foot_sequence[max_no+i-1][3] > 0)
		  	makeMovingMarker(position, s, false, true);
		    else
			makeMovingMarker(position, s, false, false);
	}
...
int main(int argc, char** argv)
{
  ...
  //menu_handler.insert( "Test Option", &processFeedback );
  ...
  goal_location_pub_ = n.advertise<interactive_walk_planner::goal_pose>("/interactive_walk_planner/goal_pose", 100, true );
  replan_message_pub_ = n.advertise<interactive_walk_planner::replan>("/interactive_walk_planner/replan_message", 100, true );
  ros::Subscriber sub_foot_sequence = n.subscribe<interactive_walk_planner::foot_sequence>( "/atlas/planned_foot_sequence", 100, &foot_sequenceCallback );

  //ros::Subscriber sub_ori_estimation = n.subscribe<interactive_walk_planner::orientation>( "/orientation_estimation/orientation", 100, &ori_estimationCallback );

  //pub_wrecs_step = n.advertise<atlas_ros_msgs::AtlasStepData>("/ocu/add_step_relative_left_foot", 100);
  pub_wrecs_step = n.advertise<atlas_ros_msgs::AtlasWalkParams>("/atlas/cmu_walk_params", 100);
