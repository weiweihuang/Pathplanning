...
void cloud_cb(const sensor_msgs::PointCloud2::ConstPtr& cloud)
{
  ...
  float theta2 = -2.5*M_PI/180.0;
  float theta3 = -0.0*M_PI/180.0;
...
void mapbulidFunc()
{
  ...
  while(1){
	...
	pass_out.setFilterLimits (0.20, 3.0);
	...
	pass_out.filter (*cloud_filter2);
		...
		pass_out.setInputCloud (cloud_filter2);	
		pass_out.setFilterFieldName ("z");
		pass_out.setFilterLimits (-0.05, 0.7);
		pass_out.filter (*cloud_filter3);
	}
	...
	pthread_mutex_unlock(&refreshMtx_);
      ...
      for(int i=0; i<cloud->points.size();i++){
        // hack to remove the point cloud created by robot body
	if (task_mode!=3){
		if (cloud->points[i].x>-0.1 && cloud->points[i].x < 0.4 && cloud->points[i].y >-0.4 && cloud->points[i].y <0.1 && cloud->points[i].z>0.01)
			continue;
	}
        ...
        terrain[x_index][y_index] += cloud->points[i].z+0.05;
	...
	normal[x_index][y_index][2] += normals1->at(i).normal_z;
      ...
      if (GoalposeUpdated_ && plan_method ==1){	
		...
		NoStopMapping_ = false;
	        ...
	        double fDir;
	        block_center.x = round((goal_pos_x-MIN_X)/DISTANCE);
	        block_center.y = round((MAX_Y-goal_pos_y)/DISTANCE);	
		Mat origin_image;
		...
		printf("rough_center.x = %.2f, rough_center.y = %.2f, \n", block_center.x, block_center.y);
			...
			block_center_f.x = block_center.x*DISTANCE+ MIN_X;
			block_center_f.y = MAX_Y - block_center.y*DISTANCE;
			...
			foot_sequence.x[1] = right_foot.x;
			foot_sequence.y[1] = right_foot.y;	
			foot_sequence.theta[1] = -fDir;
			foot_sequence.s[1] = 1;	
			foot_sequence.height[1] = 0.0;

			foot_sequence.normal_x[1] = 0;
			foot_sequence.normal_y[1] = 0;
			foot_sequence.normal_z[1] = 1;

			foot_sequence.x[2] = left_foot.x;
			foot_sequence.y[2] = left_foot.y;	
			foot_sequence.theta[2] = -fDir;
			foot_sequence.s[2] = 0;	
			foot_sequence.height[2] = 0.0;

			foot_sequence.normal_x[2] = 0;
			foot_sequence.normal_y[2] = 0;
			foot_sequence.normal_z[2] = 1;
  ...
  int main(int argc, char **argv)
  {
    ...
    ros::Subscriber sub_cloud_ = nh.subscribe<sensor_msgs::PointCloud2>("/assembled_cloud", 100, &cloud_cb);
