  ...
  if (msg->header.stamp.toSec() < initTime.toSec()) {
    printf("\n\n\n\nbogus message, %g %g\n\n\n\n\n", msg->header.stamp.toSec(), initTime.toSec());
	  ...
	  for (int i=0; i<MAX_FOOT_NO; i++){
	    ...
	    if (i>0){
/*
		if (foot_sequence[i][4] - foot_sequence[i-1][4] >0.02 && slope_phase==0)
			slope_phase = 1;
		else if (foot_sequence[i][4] - foot_sequence[i-1][4] <-0.01 && slope_phase==1)
			slope_phase = 2;
		else if (foot_sequence[i][4] - foot_sequence[i-1][4] <0.01 && foot_sequence[i][0]-foot_sequence[i-1][0] > 0.2 && slope_phase==1)
			slope_phase = 2;
		else if (fabsf(foot_sequence[i][4] - foot_sequence[i-1][4]) <0.01 && slope_phase==2)
			slope_phase = 0;
*/
