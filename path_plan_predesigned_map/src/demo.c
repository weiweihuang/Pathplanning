...
float action4_l[4][3];
float action8_l[8][3];
float action16_l[16][3]; 
float action4_r[4][3];
float action8_r[8][3];
float action16_r[16][3]; 
...
void load_actions(){
	...
	// Read txt file
	FILE *my_stream;
	my_stream=fopen("actionset4_ls.txt", "r");
	if (my_stream==NULL){
		...
		for(i=0; i<4; i++){
			...
			action4_l[i][0]=f1;
			action4_l[i][1]=f2;
			action4_l[i][2]=f3-M_PI/2;
			//printf("%6.4f %6.4f %6.4f\n", action4_l[i][0], action4_l[i][1], action4_l[i][2] );
		}
		fclose(my_stream);
	}
	my_stream=fopen("actionset8_ls.txt", "r");
	if (my_stream==NULL){
		printf("Cannot open the file");
		fclose(my_stream);
	}
		...
		for(i=0; i<8; i++){
			...
			action8_l[i][0]=f1;
			action8_l[i][1]=f2;
			action8_l[i][2]=f3-M_PI/2;
			//printf("%6.4f %6.4f %6.4f\n", action8_l[i][0], action8_l[i][1], action8_l[i][2] );
		}
		fclose(my_stream);
	}
	my_stream=fopen("actionset16_ls.txt", "r");
	if (my_stream==NULL){
		printf("Cannot open the file");
		fclose(my_stream);
	}
		...
		for(i=0; i<16; i++){
			...
			action16_l[i][0]=f1;
			action16_l[i][1]=f2;
			action16_l[i][2]=f3-M_PI/2;
			//printf("%6.4f %6.4f %6.4f\n", action16_l[i][0], action16_l[i][1], action16_l[i][2] );
		}
		fclose(my_stream);
	}

	my_stream=fopen("actionset4_rs.txt", "r");
	if (my_stream==NULL){
		printf("Cannot open the file");
		fclose(my_stream);
	}
		...
		for(i=0; i<4; i++){
			...
			action4_r[i][0]=f1;
			action4_r[i][1]=f2;
			action4_r[i][2]=f3-M_PI/2;
			//printf("%6.4f %6.4f %6.4f\n", action4_r[i][0], action4_r[i][1], action4_r[i][2] );
		}
		fclose(my_stream);
	}
	my_stream=fopen("actionset8_rs.txt", "r");
	if (my_stream==NULL){
		printf("Cannot open the file");
		fclose(my_stream);
	}
		...
		for(i=0; i<8; i++){
			...
			action8_r[i][0]=f1;
			action8_r[i][1]=f2;
			action8_r[i][2]=f3-M_PI/2;
			//printf("%6.4f %6.4f %6.4f\n", action8_r[i][0], action8_r[i][1], action8_r[i][2] );
		}
		fclose(my_stream);
	}
	my_stream=fopen("actionset16_rs.txt", "r");
	if (my_stream==NULL){
		printf("Cannot open the file");
		fclose(my_stream);
	}
		...
		for(i=0; i<16; i++){
			...
			action16_r[i][0]=f1;
			action16_r[i][1]=f2;
			action16_r[i][2]=f3-M_PI/2;
			//printf("%6.4f %6.4f %6.4f\n", action16_r[i][0], action16_r[i][1], action16_r[i][2] );
		}
		fclose(my_stream);
	}
}
...
float astar_swing_cost(float x1, float y1, float x2, float y2, ASTAR *aa)
{
	int i, j, index;
	float swing_cost, det_x, det_y, max, max_11, max_12, max_21, max_22;
	float x, y, k, x_temp, y_temp, x_0, y_0, d;
	int policy = 0;
	...
	x = x1;
	y = y1;
	while (fabsf(x-x2)>0.04||fabsf(y-y2)>0.04){
		terrain_indices( aa->tt, x, y, NULL, NULL, &index);
		if (aa->tt->perceived_cost_map[index]>max)
			max = aa->tt->perceived_cost_map[index];
		x = x + det_x;
		y = y + det_y;
	}
	if (max <0.2)
	...
	if (policy == 1){
		...
		det_y = (y_0-y1)/10;
		max_11 = 0;
		while (fabsf(x-x_0)>0.04||fabsf(y-y_0)>0.04){
			terrain_indices( aa->tt, x, y, NULL, NULL, &index);
			if (aa->tt->perceived_cost_map[index]>max_11)
				max_11 = aa->tt->perceived_cost_map[index];
			x = x + det_x;
			y = y + det_y;
		}
		x = x_0;
		...
		if (max_11 <0.2)
			...
			while (fabsf(x-x_0)>0.04||fabsf(y-y_0)>0.04){
				terrain_indices( aa->tt, x, y, NULL, NULL, &index);
				if (aa->tt->perceived_cost_map[index]>max_12)
					max_12 = aa->tt->perceived_cost_map[index];
				x = x + det_x;
				y = y + det_y;
			}
			x = x_0;
	...
	if (policy == 2){
		...
		while (fabsf(x-x_0)>0.04||fabsf(y-y_0)>0.04){
			terrain_indices( aa->tt, x, y, NULL, NULL, &index);
			if (aa->tt->perceived_cost_map[index]>max_21)
				max_21 = aa->tt->perceived_cost_map[index];
			x = x + det_x;
			y = y + det_y;
		}
		x = x_0;
		...
		if (max_21 <0.2)
			...
			while (fabsf(x-x_0)>0.04||fabsf(y-y_0)>0.04){
				terrain_indices( aa->tt, x, y, NULL, NULL, &index);
				if (aa->tt->perceived_cost_map[index]>max_22)
					max_22 = aa->tt->perceived_cost_map[index];
				x = x + det_x;
				y = y + det_y;
			}
			x = x_0;
...
float astar_step_cost( ASTAR *aa, ASTAR_NODE *an_child, ASTAR_NODE *an )
{
  ...
  swing_cost = 0;//astar_swing_cost(p_s_o[XX], p_s_o[YY], f_s_o[XX], f_s_o[YY], aa);
  ...
  x = f_s[XX] - p_s[XX];
  y = f_s[YY] - p_s[YY];
  p_s_l = sqrtf((c_s[YY] - p_s[YY])*(c_s[YY] - p_s[YY]));
  ...
  E_s = 4*(C*s_w*s_w + E_s_a);
  E_r = 0.2*ori_shift*ori_shift/l;

  return ((E_m + E_s + E_r)/3)+swing_cost;
	
}

/*******************************************************************/
float Get_terrain_cost( ASTAR *aa, ASTAR_NODE *an, float x, float y, float angle_factor)
{
  int ix, iy, index;
  float px_c, py_c, px, py, rough, sum, max, s, c, angle;
  float value[15];

  sum = 0;
  angle = an->state[ANGLE] + angle_factor-M_PI/2;
  ...
  //printf("node %g %g step cost is: %g\n", an->state[XX] , an->state[YY] , (float) ((STEP_COST_WEIGHT*(max+2*rough+sum/15))/2));
  ...
  if (max>0.1)
	  ...
	  return (float) (STEP_COST_WEIGHT*(max+2*rough+sum/15))/2;
}
...
float astar_terrain_cost( ASTAR *aa, ASTAR_NODE *an )
{
  ...
  sum = 0;
	...
	s = sinf( an->state[ANGLE]-M_PI/2 );
	c = cosf( an->state[ANGLE]-M_PI/2 );
	//s = 0;
	//c = 1;
  ...
  rough = rough/15;
  //printf("node %g %g terrain cost is: %g\n", an->state[XX] , an->state[YY] , (float) (sum));
  if (max>0.1)
	  return BIG_COST;
  else if (rough > 0.05)
	  return BIG_COST;
...
void next_target_location(ASTAR *aa, ASTAR_NODE *an, ASTAR_NODE *an_child, float x_offset, float y_offset, float rotation){
  ...
  int found_it = 0;
  float best_x, best_y, best_a, best_value;
  float value;
  ...
  xx = target_x = foot_x + c_s[XX];
  yy = target_y = foot_y + c_s[YY];


  value = Get_terrain_cost(aa, an, xx, yy, rotation);


  an_child->state[XX] = target_x;
  an_child->state[YY] = target_y;
  an_child->terrain_cost = value;
...
void straight_to_goal( ASTAR *aa, ASTAR_NODE *an, ASTAR_NODE *an_child, float y_offset, float width )
{
  ...
  // find the start foot
  foot_x = an->state[XX] + sinf( an->com[ANGLE])*y_offset;
  foot_y = an->state[YY] - cosf( an->com[ANGLE])*y_offset;
  ...
  for ( i = 0; i < n/2; i++ ){
     for ( a = 0; a <3; a++){	
	   ...
	   for ( k = -1; k <= 1; k += 2 ){
	      ...
	      if ( x < aa->tt->min[XX]
		   ...
		   || y > aa->tt->max[YY] )
			continue;
	      value = Get_terrain_cost(aa, an, x, y, a);
	      if ( value < best_value ){
			best_value = value;
			best_x = x;
			best_y = y;
	...
	break;
    if (count<0){
      xx = target_x - (i+1)*delta_x/2.0f;
      yy = target_y - (i+1)*delta_y/2.0f;
    }
    else{
      xx = target_x + i*delta_x/2.0f;
      yy = target_y + i*delta_y/2.0f;
    }
    count = -1*count;
  }
  ...
  if ( best_value < BIG_COST )
    {
      ...
      s = an_child->com[YY]-an->com[YY];
      c = an_child->com[XX]-an->com[XX];
      an_child->com[ANGLE] = atan2f( s, c );
      if (best_a == 0)
      	an_child->state[ANGLE] = an_child->com[ANGLE];
      else if (best_a == 1)
	an_child->state[ANGLE] = an_child->com[ANGLE]-10.0f*M_PI/180.0f;
      else
	an_child->state[ANGLE] = an_child->com[ANGLE]+10.0f*M_PI/180.0f;
  ...
  else
    {
      an_child->state[XX] = target_x;
      an_child->state[YY] = target_y;
      an_child->terrain_cost = BIG_COST;
	   ...
	   float angle, float width )
{
  ...
  // find the start foot
  foot_x = an->state[XX] + sinf( an->state[ANGLE])*y_offset;
  foot_y = an->state[YY] - cosf( an->state[ANGLE])*y_offset;
...
ASTAR_NODE *generate_4_children( ASTAR *aa, ASTAR_NODE *an, int *i_child )
{
  ...
  if (*i_child < 4)
    {
      if (an_child->state[SIDE] == LEFT)	
     	next_target_location( aa, an, an_child, action4_l[*i_child][0], action4_l[*i_child][1], action4_l[*i_child][2]);
      else
	next_target_location( aa, an, an_child, action4_r[*i_child][0], action4_r[*i_child][1], action4_r[*i_child][2]);
    }
   else{
      fprintf( stderr, "unknown n_child: %d\n", *i_child );
      exit( -1 );
...
ASTAR_NODE *generate_8_children( ASTAR *aa, ASTAR_NODE *an, int *i_child )
{
  ...
  if (*i_child < 8)
    {
      if (an_child->state[SIDE] == LEFT)	
      	next_target_location( aa, an, an_child, action8_l[*i_child][0], action8_l[*i_child][1], action8_l[*i_child][2]);
      else
 	next_target_location( aa, an, an_child, action8_r[*i_child][0], action8_r[*i_child][1], action8_r[*i_child][2]);
    }
   else{
      fprintf( stderr, "unknown n_child: %d\n", *i_child );
      exit( -1 );
...
ASTAR_NODE *generate_child( ASTAR *aa, ASTAR_NODE *an, int *i_child )
{
  ...
  an_child->one_step_cost = 0.7; //1*astar_step_cost( aa, an_child, an);
